...
export class HeroEffect2 {
  constructor(options = {}) {
    ...
    if (this.options.container) {
      ...
      if (shouldLog()) {
        console.error("Effect2: No container element found");
      }
    }
  }
  ...
  async loadTextures() {
    if (shouldLog()) {
      console.log("Effect2: Loading textures...");
    }
    ...
    try {
        ...
        textureLoader.load(
          ...
          (texture) => {
            ...
            if (texture.image) {
              ...
              if (shouldLog()) {
                console.log(
                  `Effect2: Image aspect ratio: ${this.textureAspectRatio}`
                );
              }
            }

            if (shouldLog()) {
              console.log("Effect2: Image texture loaded");
            }
            resolve(texture);
          },
          undefined,
          (error) => {
            if (shouldLog()) {
              console.error("Effect2: Error loading image texture:", error);
            }
            reject(error);
          }
        );
      ...
      this.depthTexture = await new Promise((resolve, reject) => {
        textureLoader.load(
          ...
          (texture) => {
            ...
            if (shouldLog()) {
              console.log("Effect2: Depth map texture loaded");
            }
            resolve(texture);
          },
          undefined,
          (error) => {
            if (shouldLog()) {
              console.error("Effect2: Error loading depth map texture:", error);
            }
            reject(error);
          }
        );
      ...
      this.edgeTexture = await new Promise((resolve, reject) => {
        textureLoader.load(
          this.options.edgeMapUrl,
          (texture) => {
            texture.minFilter = THREE.LinearFilter;
            texture.generateMipmaps = false;
            if (shouldLog()) {
              console.log("Effect2: Edge map texture loaded");
            }
            resolve(texture);
          },
          undefined,
          (error) => {
            if (shouldLog()) {
              console.error("Effect2: Error loading edge map texture:", error);
            }
            reject(error);
          }
        );
      ...
      this.uniforms.uEdgeMap.value = this.edgeTexture;
      if (shouldLog()) {
        console.log("Effect2: All textures loaded successfully");
      }

      this.updateCanvasSize();
    } catch (error) {
      if (shouldLog()) {
        console.error("Effect2: Error loading textures:", error);
      }
    }
  }
    ...
    this.material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          vec4 edgeMap = texture2D(uEdgeMap, vUv);
          ...
          float flow = 1.0 - smoothstep(0.0, 0.02, abs(depthMap.r - uProgress));
          
          vec3 mask = (1.0 - edgeMap.rgb) * flow * uColor * uIntensity * 1.5;
  ...
  startScanAnimation() {
    if (shouldLog()) {
      console.log("Effect2: Starting scan animation");
    }
    ...
    const animate = () => {
      ...
      // Much slower scanning - from 0.007 to 0.0007 (10x slower)
      this.uniforms.uProgress.value += 0.0007;
              ...
              if (!this.fallbackAnimationActive) {
                ...
                if (!this.fallbackAnimationId) {
                  const animate = () => {
                    ...
                    this.uniforms.uProgress.value += 0.007;
                    if (this.uniforms.uProgress.value > 1) {
                      this.uniforms.uProgress.value = 0;
                    }
                    this.fallbackAnimationId = requestAnimationFrame(animate);
...
export function initHeroEffect2(options = {}) {
  ...
  if (!mergedOptions.imageUrl) {
    if (shouldLog()) {
      console.error("Effect2: No image URL provided");
    }
    return null;
  }
  ...
  if (!mergedOptions.depthMapUrl) {
    if (shouldLog()) {
      console.error("Effect2: No depth map URL provided");
    }
    return null;
  }

  if (!mergedOptions.edgeMapUrl) {
    if (shouldLog()) {
      console.error("Effect2: No edge map URL provided");
    }
    return null;
  }

  return new HeroEffect2(mergedOptions);
}
